...
long  vel_retract_mot9 = vel / 5;
long  vel_retract_mot8 = vel / 5;
long  vel_gancho  = vel;
long  vel_feed   = vel/4;
...
//      }
  ...
  **************/
  ...
  //Motor 8 se mueve hasta limit switch trasero   
    //while (!digitalRead(LS_tras)) {
    //  stepper8.run();
     // if (digitalRead(LS_izq) || digitalRead(LS_cort_izq) || digitalRead(LS_cort_der)) { // || !digitalRead(LS_der)
      //  warning = 9;
      //  break;
    //  }
   // }
  ...
  //  }
    ...
    while (true) {
      if (digitalRead(LS_izq) || digitalRead(LS_tras) || digitalRead(LS_frnt) || digitalRead(LS_cort_izq) || digitalRead(LS_cort_der)) { //|| !digitalRead(LS_der)
        warning = 9;
        break;
      }
      stepper2.run();
