...
int steps_gancho_der  = 1200;        // 1.25 mm/vuelta.
int steps_gancho_corte = 200;   // 1.25 mm/vuelta.
int steps_cable = 202 / 8 * 200 ; // 8 mm/vuelta.   
int steps_enrol = -245;          // una vuelta considerando reducción 
int steps_meand = 600/3;          // tres vueltas
int steps_meand_fin = 200;          // una vueltas
int steps_solid_enrol = 200 * 63 / 4; //steps_enrol*63/4; //Por cada vuelta del motor 4 y 5 se avanzan 63*2 mm. Entonces, el mot9 necesita 63*2/8 vueltas (8 mm por vuelta mot 9)
...
int steps_solid_meand = (steps_meand * 8 / 4);  //steps_meand*8/2;    //Por cada vuelta del motor 6 y 7 se avanzan ((13-5)*2)*2 mm. Entonces, el mot9 necesita  8*2*2/8 vueltas. EMPIRICO
// - (5 * steps_meand / 100 * 200 / 8);
int steps_enrol_demas = -13; // 1/20 de vuelta del motor 4 o 5 aprox.
...
void setup() {
  ...
  stepper2.moveTo(160);    //DERECHO. positivo abre, negativo cierra
  stepper3.setMaxSpeed(vel_gancho);
  stepper3.setAcceleration(acel_gancho);
  stepper3.moveTo(160);    //IZQUIERDO. positivo abre, negativo cierra
  
  // setup para ajustar motores 4 y 5 
  stepper4.setMaxSpeed(vel_enrol);
  stepper4.setAcceleration(acel);
  stepper4.moveTo(600);    //dirección en funcionamiento normal: positiva.
  stepper5.setMaxSpeed(vel_enrol);
  stepper5.setAcceleration(acel);
  stepper5.moveTo(-600);    //dirección en funcionamiento normal: negativa.
  ...
  stepper6.moveTo(-200);    //dirección en funcionamiento normal: negativa. steps_meand. DERECHO
  stepper7.setMaxSpeed(vel_meand);
  stepper7.setAcceleration(acel);
  stepper7.moveTo(200);    //dirección en funcionamiento normal: positiva. steps_meand. IZQUIERDO
...
void ejecutar(int Paso) {
  ...
  **************/
  ...
  if (Paso == 8) { // CAMBIAR A 9
    ...
    while (!digitalRead(LS_tras)) {
      stepper8.run();
      if (digitalRead(LS_izq) || !digitalRead(LS_der) || digitalRead(LS_gan_izq) || digitalRead(LS_gan_der)) {
        warning = 9;
        break;
      }
    }
    ...
    while (true) {
      if (digitalRead(LS_izq) || digitalRead(LS_tras) || digitalRead(LS_frnt) || !digitalRead(LS_der) || digitalRead(LS_gan_izq) || digitalRead(LS_gan_der)) {
        warning = 9;
        break;
      }
      stepper2.run();
